...
- 트랙 목록에서 **🔀 랜덤 재생**을 선택하면 해당 플레이리스트의 트랙 중 하나를 무작위로 재생하고, 곡이 끝나면 자동으로 다른 트랙을 무작위로 이어서 재생합니다
- 플레이리스트 옆 ⭐ 버튼으로 현재 선택된 플레이리스트를 즐겨찾기에 등록/해제할 수 있고, **즐겨찾기한 플레이리스트만 보기** 체크박스로 플레이리스트 목록을 즐겨찾기한 것만 보이도록 필터링할 수 있습니다 (즐겨찾기는 GM 브라우저에 개인적으로 저장됩니다)
...
Foundry는 모듈을 자동으로 갱신하지 않습니다. 새 버전을 반영하려면:

1. 월드 밖 **Setup** 화면 → **Add-on Modules** 목록에서 BGM 마스터의 **Update** 버튼을 눌러 새 버전을 내려받습니다.
2. 월드를 다시 시작(재접속)합니다.
3. 그래도 UI가 그대로면 브라우저 캐시 때문일 수 있으니 `Ctrl+Shift+R`(하드 리프레시)로 새로고침합니다.
...
3. 리모컨에서 **Playlist**를 선택하면 Track은 기본으로 **🔀 랜덤 재생**이 지정됩니다(원하면 Track에서 특정 곡으로 바꿀 수 있습니다).
